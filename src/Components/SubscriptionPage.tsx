...
import { CheckCircle, CreditCard, AlertCircle, Sparkles, Crown, Check, Lock } from "lucide-react";
...
// ✅ Demo plans for when backend is down
...
const SubscriptionPage: React.FC<SubscriptionPageProps> = ({ 
  ...
  useEffect(() => {
    const fetchPlans = async () => {
      try {
        ...
        if (isFirstTime) {
          const trialPlan = res.data.find((p: Plan) => p.price === 0);
          if (trialPlan) setSelectedPlan(trialPlan);
        }
      } catch (err) {
        console.log("⚠️ Backend not available, using demo plans");
        // ✅ Fallback to demo plans
        setPlans(DEMO_PLANS);
        if (isFirstTime) {
          setSelectedPlan(DEMO_PLANS[0]); // Auto-select trial
        }
      }
    };
    fetchPlans();
  ...
  const handleActivatePlan = async () => {
    ...
    if (isFirstTime && selectedPlan.price > 0) {
      setError("Please start with the free 30-day trial first!");
      return;
    }
    ...
    try {
      if (selectedPlan.price === 0) {
        ...
        // Try backend (optional, won't fail if down)
        try {
        ...
        // ✅ Call onComplete instead of navigate
        setTimeout(() => {
          if (onComplete) {
            onComplete();
          }
      ...
      // ✅ Fallback: Even on error, activate locally if trial
      if (selectedPlan.price === 0) {
        localStorage.setItem('has_seen_subscription', 'true');
        localStorage.setItem('subscription_status', 'active');
        setPaymentSuccess(true);
...
const SubscriptionWrapper: React.FC<SubscriptionPageProps> = ({ isFirstTime, userId, onComplete }) => (
  <Elements stripe={stripePromise}>
    <SubscriptionPage isFirstTime={isFirstTime} userId={userId} onComplete={onComplete} />
  </Elements>
);
